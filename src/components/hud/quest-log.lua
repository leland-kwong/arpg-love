...
local function layoutQuests(log, wrapLimit, font)
  local calcCompleted = function(completeCount, task)
    return completeCount + (log.completedTasks[task.id] and 1 or 0)
  end

  local questsList = F.reduce(F.keys(log.quests), function(list, questId)
    table.insert(list, log.quests[questId])
    return list
  end, {})
  ...
  local layout = F.reduce(questsList, function(layout, quest)
    ...
    F.forEach(quest.subTasks, function(task)
      ...
      local isCompleted = log.completedTasks[task.id]
      local checkIcon = (isCompleted and glyphs.checkboxChecked or glyphs.checkbox) .. ' '
      ...
      table.insert(formattedString, 1, isCompleted and colors.completed or Color.WHITE)
...
function QuestLog.init(self)
  ...
  self.guiNodes = {
    {
      Gui.create({
        ...
        onUpdate = function(self)
          ...
          local offsetY = 16
          if (#F.keys(parent.log.quests) > 0) then
            local rect = layoutQuests(parent.log, wrapLimit, bodyText.font)
  ...
  self.logDisplay = MenuList2.create({
    x = self.x,
    y = self.y,
...
function QuestLog.update(self)
  ...
  self.logDisplay.width = self.width
end
...
function QuestLog.draw(self)
  ...
  love.graphics.rectangle('fill', self.x - padding, self.y - padding, self.width + padding*2, self.logDisplay.height + padding*2)
end

function QuestLog.drawOrder(self)
  return self.logDisplay:drawOrder() - 1
end
