...
local frozenShatterEffect = function(particleImage, particleImageQuad)
  return function(startX, startY, startZ, speed, numParticlesMin, numParticlesMax, sizeMin, sizeMax, lifeTime)
    ...
    for i=1, numParticles do
      ...
      local dist = length(initialX, initialY, endX, endY)
      table.insert(
        particles,
        {
          curve = curve,
    ...
    local function drawIceCube(p, x, y, _, _, sx, sy)
      local lifeRatio = p.lifeTime/lifeTime
      local opacity = 1 - lifeRatio
      love.graphics.setColor(0.2,0.7,1,opacity)
    ...
    return function(dt)
      ...
      for i=1, numParticles do
        local p = particles[i]
        local actualDt = dt / speed / (p.dist / 100)
...
local whitePixel = defaultTexture
local frozenShatterParticleSystem = love.graphics.newParticleSystem(whitePixel, 500)
...
return frozenShatterEffect(
  AnimationFactory.atlas,
  AnimationFactory:new({'cube'})
)